# Test script to verify gray shading for mouse 157474
library(DBI)
library(RSQLite)
...
# Test date range calculation (simulating the enhanced logic)
if (nrow(body_weight_data) > 0) {
  ...
  cat("\nEnhanced plot range (before padding):\n")
  cat("From:", as.character(plot_date_range[1]), "to", as.character(plot_date_range[2]), "\n")
  
  # Add padding calculation
  date_range_span <- as.numeric(plot_date_range[2] - plot_date_range[1])
  cat("Date range span (days):", date_range_span, "\n")
  
  if (date_range_span == 0) {
    padding_days <- 3
    cat("Single day detected, using 3 days padding\n")
  } else {
    padding_days <- max(2, min(7, ceiling(date_range_span * 0.1)))
    cat("Multiple days, calculated padding:", padding_days, "days\n")
  }
  
  plot_date_range[1] <- plot_date_range[1] - padding_days
  plot_date_range[2] <- plot_date_range[2] + padding_days
  
  cat("\nFinal plot range (with padding):\n")
  cat("From:", as.character(plot_date_range[1]), "to", as.character(plot_date_range[2]), "\n")
  
  # Test if gray shading rectangle would be created
  cat("\nGray shading analysis:\n")
  for (i in seq_len(nrow(plugging_data))) {
    row <- plugging_data[i, ]
    start_date <- as.Date(row$pairing_start_date)
    end_date <- as.Date(row$pairing_end_date)
    ...
    cat("Rectangle", i, ":\n")
    cat("  x0 (start):", as.character(start_date), "\n")
    cat("  x1 (end):", as.character(end_date), "\n")
    cat("  Within plot range:", start_date >= plot_date_range[1] && end_date <= plot_date_range[2], "\n")
    cat("  Rectangle width (days):", as.numeric(end_date - start_date), "\n")
  }
}
