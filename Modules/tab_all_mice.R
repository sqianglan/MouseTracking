...
all_mice_tab_server <- function(input, output, session, all_mice_table, is_system_locked = NULL, global_refresh_trigger = NULL, shared_plugging_state = NULL) {
  ...
  # Body weight functionality - single event handler for all buttons
  observeEvent(input$add_body_weight_clicked, {
    asu_id <- input$add_body_weight_clicked
    if (!is.null(asu_id) && asu_id != "") {
      show_body_weight_input(input, output, session, asu_id)
    }
  }, ignoreInit = TRUE)
  
  # Body weight save functionality
  observeEvent(input$save_body_weight_clicked, {
    req(input$save_body_weight_clicked)
    
    asu_id <- input$save_body_weight_clicked
