# Test script to verify gray shading fix for mouse 157210
library(DBI)
library(RSQLite)
...
if (nrow(body_weight_data) > 0) {
  ...
  cat("\nEnhanced plot range (including all pairing periods):\n")
  cat("From:", as.character(plot_date_range[1]), "to", as.character(plot_date_range[2]), "\n")
  
  cat("\nRange expansion needed:", 
      plot_date_range[1] < original_range[1] || plot_date_range[2] > original_range[2], "\n")
  cat("Days covered:", as.numeric(plot_date_range[2] - plot_date_range[1]) + 1, "\n")
}
